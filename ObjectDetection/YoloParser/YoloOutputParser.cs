...
class YoloOutputParser
{
    ...
    static ValueTuple<int, float> GetTopResult(float[] predictedClasses) =>

        predictedClasses.Select((predictedClass, index) => (Index: index, Value: predictedClass))
                        .OrderByDescending(result => result.Value)
                        .First();

    static float IntersectionOverUnion(RectangleF boundingBoxA, RectangleF boundingBoxB)
    {
    ...
    int GetOffset(int x, int y, int channel) =>
        ...
        channel * channelStride + y * COL_COUNT + x;
    ...
    static readonly Color[] classColors = new[]
    {
        Color.Khaki,
        Color.Fuchsia,
        Color.Silver,
    ...
    readonly int channelStride = ROW_COUNT * COL_COUNT;
}